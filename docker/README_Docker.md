...
If you want a different password on first boot, set `WIREBUDDY_BOOTSTRAP_ADMIN_PASSWORD` explicitly.
...
services:
  wirebuddy:
    ...
    environment:
      ...
      # Optional: override the default bootstrap admin password on first boot
...
| `WIREBUDDY_BOOTSTRAP_ADMIN_PASSWORD` | No | `admin` on first boot | Overrides the initial password for the bootstrap `admin` user when the database is created |
