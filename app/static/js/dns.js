...
(function () {
    ...
    if (!WBShared.RefreshScheduler) {
        throw new Error('DNS page requires WBShared.RefreshScheduler');
    }
    ...
    let _trendLoadInProgress = false;
    let _trendLoadPending = false;
    let _topDomainsLoadInProgress = false;
    let _topDomainsLoadPending = false;
    ...
    function _onPageShow(ev) {
        ...
        if (!_adBlockerEnabled && _adblockerDisabledUntil > now) {
    ...
    async function loadTopDomains() {
        if (_topDomainsLoadInProgress) {
            _topDomainsLoadPending = true;
            return;
        }

        _topDomainsLoadInProgress = true;
        try {
            do {
                _topDomainsLoadPending = false;
            ...
            } while (_topDomainsLoadPending);
        } catch (e) {
            if (!isAbortError(e)) {
                console.error('Top domains error:', e);
            }
        } finally {
            _topDomainsLoadInProgress = false;
        }
    }
    ...
    const _refreshScheduler = new WBShared.RefreshScheduler({
