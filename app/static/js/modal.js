...
function _showModal({ title, message, type, showCancel, showInput, inputDefault, inputPlaceholder, inputType }) {
    ...
    const previousModalController = _activeModalController;
    if (previousModalController) {
        previousModalController.finish?.();
        previousModalController.abort?.();
        if (_activeModalController === previousModalController) {
