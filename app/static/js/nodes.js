...
(function () {
    ...
    function renderEnrollmentToken(container, token) {
        if (!container) return;

        container.textContent = '';
        const tokenValue = document.createElement('code');
        tokenValue.className = 'enrollment-token-value';
        tokenValue.textContent = String(token || '');
        container.appendChild(tokenValue);
    }
    ...
    function initNodeActionDropdowns(root = document) {
        ...
        for (const trigger of scope.querySelectorAll('.node-actions-more-toggle')) {
            ...
            // Configure dropdown to escape table-responsive clipping by using fixed strategy
    ...
    function closeNodeActionDropdowns(clickedInside = null) {
        if (typeof bootstrap === 'undefined' || !bootstrap.Dropdown) {
            return;
        }

        for (const trigger of document.querySelectorAll('.node-actions-more-toggle[aria-expanded="true"]')) {
    ...
    async function submitAddNode() {
        ...
        try {
            ...
            if (enrollmentTokenDisplay) {
                renderEnrollmentToken(enrollmentTokenDisplay, data.enrollment_token);
            }
            if (addNodeSubmitBtn) {
    ...
    async function regenerateToken(id, name) {
        ...
        try {
            ...
            if (tokenModalEl && regenTokenDisplay) {
                renderEnrollmentToken(regenTokenDisplay, data.enrollment_token);
    ...
    document.addEventListener('click', (event) => {
        const clickedInsideDropdown = event.target.closest('.node-actions-more');
        if (!clickedInsideDropdown) {
            closeNodeActionDropdowns();
        }
